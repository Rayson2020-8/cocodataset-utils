...
class Analyzer:
    def __init__(self, coco_path, save_path=None, map_size=8):
        ...
        self.__coco = COCO(coco_path)
        self.__map_size = map_size
        self.__s_threshold = 32 * 32
        self.__m_threshold = 96 * 96
        self.__drawUtil = DrawUtils(save_path)
        self.__create_data()
        self.__draw()

    def __create_data(self):
        img_ids = self.__coco.getImgIds()
        ano_ids = self.__coco.getAnnIds()
        cat_ids = self.__coco.getCatIds()
        imgs_num = len(img_ids)
        anos_num = len(ano_ids)
        cat_num = len(cat_ids)
        self.__imgs_data = np.zeros((imgs_num, 4))
        self.__anos_data = np.zeros((anos_num, 8))
        self.__cats_data = np.zeros((cat_num, 2))
        temp_ano_i = 0
        for ig_id in img_ids:
            img_sample = self.__coco.loadImgs(ig_id)[0]
            self.__imgs_data[ig_id, 0] = img_sample["width"]
            self.__imgs_data[ig_id, 1] = img_sample["height"]
            img_area = img_sample["width"] * img_sample["height"]
            relate_anos_ids = self.__coco.getAnnIds(ig_id)
            self.__imgs_data[ig_id, 3] = len(relate_anos_ids)
            for ano_id in relate_anos_ids:
                ano_sample = self.__coco.loadAnns(ano_id)[0]
                self.__anos_data[temp_ano_i, 0] = ano_sample["area"]
                self.__anos_data[temp_ano_i, 1] = self.__calObjSize(ano_sample["area"], self.__s_threshold, self.__m_threshold)
                self.__anos_data[temp_ano_i, 2] = ano_sample['area'] / img_area
                self.__anos_data[temp_ano_i, 3] = ano_sample["bbox"][3] / ano_sample["bbox"][2]
                self.__anos_data[temp_ano_i, 4] = ano_sample["category_id"]
                self.__anos_data[temp_ano_i, 5], self.__anos_data[temp_ano_i, 6] = self.__calMapIndex(ano_sample["bbox"], \
                                                                                   self.__map_size,      \
                                                                                   img_sample["height"], \
                                                                                   img_sample["width"])
                self.__anos_data[temp_ano_i, 7] = ano_sample["image_id"]
                temp_ano_i += 1
        for c_id in cat_ids:
            self.__cats_data[:, 0] = self.__coco.getImgIds(catIds=[c_id])[0]
            self.__cats_data[:, 1] = sum(self.__anos_data[:, 4] == c_id)
    ...
    def __calOverallInformation(self):
        ...
        values = [self.__imgs_data.shape[0], self.__anos_data.shape[0], self.__cats_data.shape[0]]
        return labels, values

    def __calImgSize(self):
        labels = []
        values = []
        unique_ws = np.unique(self.__imgs_data[:, 0])
        for u_w in unique_ws:
            realate_indexs = np.where(self.__imgs_data[:, 0]==u_w)
            realte_hs = self.__imgs_data[:, 1][realate_indexs]
    ...
    def __calImgObjNumber(self):
        data = collections.Counter(self.__imgs_data[:, 3])
    ...
    def __calCategoryNumber(self):
        ...
        cat_ids = self.__coco.getCatIds()
        for c_i in cat_ids:
            la = self.__coco.loadCats(c_i)[0]["name"]
            val = np.where(self.__anos_data[:, 4]==c_i)[0].size
            labels.append(la)
            values.append(val)
        return labels, values
    ...
    def __calAbsArea(self):
        ...
        values = []
        for i in range(3):
            val = np.where(self.__anos_data[:, 1]==i)[0].size
            values.append(val)
        return labels, values

    def __calRealateSize(self):
        return self.__anos_data[:, 2]

    def __calMapDist(self):
        values = np.zeros((self.__map_size, self.__map_size))
        for i in range(self.__map_size):
            for j in range(self.__map_size):
                x_indexs = np.where(self.__anos_data[:, 5] == i)
                r_y = self.__anos_data[:, 6][x_indexs]
                val = np.where(r_y==j)[0].size
                values[i, j] = val
        return values

    def __calAspectRatio(self):
        return self.__anos_data[:, 3]

    def __calCatImgNumber(self):
        values = []
        cat_ids = self.__coco.getCatIds()
        label = [x["name"] for x in self.__coco.loadCats(cat_ids)]
        for c_i in cat_ids:
            index = np.where(self.__anos_data[:, 4] == c_i)
            related_img_id = self.__anos_data[:, 7][index]
